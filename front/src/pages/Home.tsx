...
const Home = () => {
  ...
  const topTags = useMemo(() => {
    ...
    for (const m of memes) {
      const tags = Array.isArray(m.tags) ? m.tags : (typeof m.tags === 'string' ? (m.tags as string).split(',').map(t => t.trim()).filter(Boolean) : []);
      for (const t of tags) {
        ...
        {filteredImages.map((img: Meme) => (
          ...
          >
            ...
            <div className="p-3 space-y-1">
              ...
              {((img.likesCount || 0) > 0 || (img.dislikesCount || 0) > 0 || (img.commentsCount || 0) > 0) && (
                <div className="flex items-center gap-3 mt-2 pt-2 border-t border-gray-100">
                  {(img.likesCount || 0) > 0 && (
                    <ReactionUsersPopover memeId={img.id} type="like">
                  ...
                  {(img.dislikesCount || 0) > 0 && (
                    <ReactionUsersPopover memeId={img.id} type="dislike" align="right">
                      <div className="flex items-center gap-1 text-xs text-gray-600">
                        <IconDislike size={16}/>
