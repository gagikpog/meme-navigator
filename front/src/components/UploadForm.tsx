...
const UploadForm = ({ onUpload }: IUploadFormProps) => {
  ...
  const setFileWithPreview = useCallback(async (newFile: File) => {
    if (!newFile) return;
    revokePreviewUrl();
    setStatus('Обработка изображения...');
    const compressIfNeededInline = async (inputFile: File) => {
      try {
        const originalSize = inputFile.size;
        const blob = inputFile instanceof Blob ? inputFile : new Blob([inputFile], { type: (inputFile as {type: string}).type || 'image/*' });
        ...
        const compressedBlob: Blob | null = await new Promise((resolve) => canvas.toBlob(resolve, targetMime, quality));
        if (!compressedBlob) {
