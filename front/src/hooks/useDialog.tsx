...
type DialogButtonType = 'yes' | 'no' | 'cancel';

type DialogButtonsConfig = { [key in DialogButtonType]?: { text?: string; className?: string; icon?: React.ReactNode | null } };
...
const DialogComponent = ({
  ...
  const getButtonConfig = (type: DialogButtonType) => {
    const configs = {
      yes: {
        text: 'Да',
  ...
  const renderButton = (type: DialogButtonType) => {
...
export const useDialog = () => {
  ...
  const show = useCallback((config: IDialogComponentProps) => {
    setDialogState({
      isOpen: true,
      title: config.title || '',
  ...
  const showModal = useCallback((config: IDialogComponentProps) => {
  ...
  const close = useCallback((result: string = 'cancel') => {
    setDialogState(prev => {
      if (prev.onClose) {
        prev.onClose(result);
      }
